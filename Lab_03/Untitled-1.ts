// Задание 1. Симулятор "Службы доставки" 
const checkStock = (item: string): Promise<string> => {
...
const processPayment = (amount: number): Promise<number> => {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            const balance = 1000;
...
const deliverOrder = (): Promise<string> => {
...
// Задание 2. Гонка запросов
const fetchFast = (): Promise<string> => {
    return new Promise((resolve) => setTimeout(() => resolve("Быстрый запрос"), 500));
};

const fetchSlow = (): Promise<string> => {
...
async function limitRequests<T>(tasks: (() => Promise<T>)[], limit: number): Promise<T[]> {
    const results: T[] = [];
...
3. resolve и reject – это функции, вызываемые внутри промиса.
