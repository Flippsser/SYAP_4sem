...
import { uiSettingsState, viewModeSelector, ViewMode } from '../state/uiSettingsState';

const styles = {
  container: { minHeight: '100vh', background: 'var(--bg-color)' },
  main: { maxWidth: 1200, margin: '0 auto', padding: 30 },
  toolbar: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 30, flexWrap: 'wrap' as const, gap: 12 },
  title: { fontSize: 28, fontWeight: 600 },
  addBtn: { padding: '12px 24px', background: '#27ae60', color: 'white', border: 'none', borderRadius: 10, fontSize: 16, cursor: 'pointer' },
  select: { padding: '10px 12px', border: '1px solid var(--border-color)', borderRadius: 10, fontSize: 14, background: 'var(--card-bg)', color: 'var(--text-color)' },
  ...
  settingsRow: { display: 'flex', gap: 12, alignItems: 'center' },
};
...
const CatalogPage: React.FC<Props> = ({ category }) => {
  ...
  const setViewMode = (mode: ViewMode) => setUiSettings((prev) => ({ ...prev, viewMode: mode }));
  const toggleTheme = () => setUiSettings((prev) => ({ ...prev, theme: prev.theme === 'light' ? 'dark' : 'light' }));
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <div style={styles.settingsRow}>
              <button
                ...
                style={viewMode === 'grid' ? styles.toggleActive : styles.toggle}
              >
                Сетка
              </button>
              <button
